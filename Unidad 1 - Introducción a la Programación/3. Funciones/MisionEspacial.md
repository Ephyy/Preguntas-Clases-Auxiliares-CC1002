...
1. Escribir una función que convierta kilómetros a unidades espaciales. Esta función se llamará
...
2. Escribir una función que calcule el combustible utilizado llamada `calcular_combustible`. Esta recibirá los argumentos `distancia_km` y `peso_nave`. Para hacer el cálculo considere que se gastan 50 litros por unidad espacial y por tonelada de nave.

3. Escribir una función llamada `calcular_costo_mision` que calcule el costo total de la misión, considere que el costo total corresponde a lo que se gasta en combustible. La función debe recibir como argumentos la distancia en km, el peso en toneladas y el precio del combustible por litro.
