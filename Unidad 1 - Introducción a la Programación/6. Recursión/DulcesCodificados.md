...
+ Cree una función recursiva ```contarDulces(n,p)``` , que dado un entero `n` y un entero `p` (con 0 <= p <= 9), entrega como resultado cuantas veces se repite el tipo de dulce `p` en el pedido codificado como el número `n`. Ejemplo: 
  - ```contarDulces(143412549,4)``` entrega ```3```

+ Cree una función ```mostrarPedido(n)``` , que recibe un número entero `n`, el cual representa un pedido, y muestra en pantalla la cantidad que se ordenó de cada tipo de dulce. Ejemplo:
