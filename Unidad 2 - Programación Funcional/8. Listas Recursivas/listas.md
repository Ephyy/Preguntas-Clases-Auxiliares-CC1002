...
a) Cree la función ```aString(L)```, que recibe una lista de elementos, y entrega un string que muestra todos los elementos de la lista, separados por un espacio. Ejemplo:
...
e) Cree la función ```obtener(L, i)```, que recibe una lista y un número entero que indica una posición en la lista, y entrega el elemento que se encuentra en tal posición. En caso de que el número se encuentre fuera del rango de la lista, entonces devuelve ```None```. Ejemplo:
