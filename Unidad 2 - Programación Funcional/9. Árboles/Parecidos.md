...
1. Cree la función ```contieneTodos(A1, A2)```, que recibe dos árboles binarios, y entrega ```True``` si es que todos los elementos del árbol `A1` se encuentran presentes en `A2`. Por ejemplo:
...
2. Cree la función ```parecidos(A1, A2)```, que recibe dos árboles binarios, y entrega ```True``` si es que tales arboles cumplen con ser parecidos. Por ejemplo:
